...
export class ProductionListComponent implements OnInit {
    ...
    delete() {
        this.productionService.delete(this.delObj.id)
                         .then(resp => this.msg.success("产品:" + resp.data.name + "已删除！")).then(resp => this.getData() )
                         .catch((error) => {this.msg.error(error); this.loading = false;})
    }

    add() {
        this.productionService.formOperation = 'create';
        this.productionService.isUpdate=false;
        this.router.navigateByUrl('/productions/form');
    }

    update(id) {
        this.productionService.formOperation='update';
        this.productionService.initUpdate(id)
            .then(result => { this.productionService.production = result.data;})
            .then(() => this.router.navigateByUrl('/productions/form')).catch((error)=>
